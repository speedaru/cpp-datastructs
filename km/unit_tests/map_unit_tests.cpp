...
void spd::unit_test::UnorderedMap() {
    ...
    {
        LOG_D("creating empty map\n");
        spd::UnorderedMap<spd::String<char>, spd::String<char>> map;
        map.SetTag("map<str, str>");

        LOG_D("constructing k1\n");
        spd::String k1("stirng1");
        k1.SetTag("k1");

        LOG_D("constructing v1\n");
        spd::String v1("value1");
        k1.SetTag("v1");

        LOG_D("inserting v1 into map at k1\n");

        map[k1] = v1;

        LOG_D("inserted v1 !\n");

        LOG_D("map[%.*s] : %.*s\n", SPD_FMT_SV(k1.Str()), SPD_FMT_SV(map[k1].Str()));
